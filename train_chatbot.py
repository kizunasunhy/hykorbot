...
from transformers import (AdamW, GPT2DoubleHeadsModel, GPT2Config, WEIGHTS_NAME, CONFIG_NAME, cached_path)
...
def add_special_tokens_(model, tokenizer):
    ...
    print('added_token_num:', num_added_tokens)
    if num_added_tokens > 0:
        model.resize_token_embeddings(new_num_tokens=tokenizer.size + num_added_tokens + 1)

def build_input_from_segments(history, reply, tokenizer, lm_labels=False, with_eos=True):
    bos, eos, sent1, sent2 = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[:-1])
    sequence = [[bos]] + history + [reply + ([eos] if with_eos else [])]
    sequence = [sequence[0]] + [[sent2 if (len(sequence)-i) % 2 else sent1] + s for i, s in enumerate(sequence[1:])]
    instance = {}
    instance["input_ids"] = list(chain(*sequence))
    instance["token_type_ids"] = [sent2 if i % 2 else sent1 for i, s in enumerate(sequence) for _ in s]
...
def make_logdir(model_name: str):
    ...
    logdir = os.path.join(
        'runs_chatbot', current_time + '_' + socket.gethostname() + '_' + model_name)
    return logdir
